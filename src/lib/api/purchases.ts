...
export async function createPurchaseOrder(input: any): Promise<any> {
  ...
  const existing = await supabaseFetch("purchase_orders", "GET", null, `?select=code&code=like.PO-${year}-%`);
  ...
  const [po] = await supabaseFetch("purchase_orders", "POST", {
  ...
  if (input.items?.length > 0 && po) {
    ...
    if (items.length > 0) await supabaseFetch("purchase_order_items", "POST", items);
  }
  return po;
}
