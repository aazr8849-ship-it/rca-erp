...
export default function OrderDetailPage() {
  ...
  const prColumns: Column<PurchaseRequest>[] = [
    { key: "code", header: "请购号", render: (r) => <Link href={`/purchases/${r.id}`} className="text-xs font-mono text-[#2E8BFF] hover:underline">{r.code}</Link> },
  ...
  const poColumns: Column<PurchaseOrder>[] = [
    { key: "code", header: "采购号", render: (r) => <Link href={`/purchases/${r.id}`} className="text-xs font-mono text-[#2E8BFF] hover:underline">{r.code}</Link> },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          {buttons.map((b) => (
            <Button key={b.status} onClick={() => handleStatusChange(b.status)} variant={b.variant} className={b.variant === "default" ? "bg-[#2E8BFF] hover:bg-[#0B4BB8]" : ""}>{b.label}</Button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <InfoCard title="订单信息" icon={ShoppingCart} className="lg:col-span-2">
          <div className="grid grid-cols-2 gap-x-4 gap-y-4">
            ...
            <InfoItem label="客户" value={<Link href={`/customers/${order.customer_id}`} className="text-[#2E8BFF] hover:underline">{order.customer_name}</Link>} />
            ...
            <InfoItem label="总金额" value={<span className="text-[#2E8BFF] font-medium">{formatCurrency(order.total_amount, order.currency)}</span>} />
        ...
        <InfoCard title="金额信息" icon={DollarSign}>
          <div className="space-y-3">
            <div className="bg-blue-50 rounded-md p-3"><div className="text-xs text-gray-600">订单总金额</div><div className="text-2xl font-bold text-[#2E8BFF] mt-1">{formatCurrency(order.total_amount, order.currency)}</div></div>
          ...
          <InfoCard>
            <Table>
              ...
              <TableBody>
                ...
                <TableRow className="bg-gray-50 font-medium"><TableCell colSpan={6} className="text-right text-sm">合计：</TableCell><TableCell className="text-right text-[#2E8BFF] text-base">{formatCurrency(order.total_amount, order.currency)}</TableCell></TableRow>
              </TableBody>
            </Table>
          </InfoCard>
        </TabsContent>
