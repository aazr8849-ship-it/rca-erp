        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <InfoCard title="采购订单信息" icon={Truck} className="lg:col-span-2">
            <div className="grid grid-cols-2 gap-x-4 gap-y-4">
              ...
              <InfoItem label="供应商" value={<Link href={`/suppliers/${po.supplier_id}`} className="text-[#2E8BFF] hover:underline">{po.supplier_name}</Link>} />
              <InfoItem label="下单日期" value={formatDate(po.order_date)} />
              <InfoItem label="币种" value={po.currency} />
              <InfoItem label="总金额" value={<span className="text-[#2E8BFF] font-medium">{formatCurrency(po.total_amount, po.currency)}</span>} />
        ...
        <InfoCard title="采购明细">
          <Table>
            ...
            <TableBody>
              ...
              <TableRow className="bg-gray-50 font-medium"><TableCell colSpan={7} className="text-right text-sm">合计：</TableCell><TableCell className="text-right text-[#2E8BFF] text-base">{formatCurrency(po.total_amount, po.currency)}</TableCell></TableRow>
            </TableBody>
          </Table>
        </InfoCard>
      </div>
    ...
    <div className="space-y-4">
      ...
      <InfoCard title="请购单信息" icon={ClipboardList}>
        <div className="grid grid-cols-2 gap-x-4 gap-y-4">
          ...
          <InfoItem label="关联订单" value={order ? <Link href={`/orders/${order.id}`} className="text-[#2E8BFF] hover:underline">{order.code}</Link> : "-"} />
