...
export default function QuotationsPage() {
  ...
  const columns: Column<Quotation>[] = [
    { key: "code", header: "报价号", width: "140px", render: (r) => <Link href={`/quotations/${r.id}`} className="text-xs font-mono text-[#2E8BFF] hover:underline">{r.code}</Link> },
    ...
    { key: "total_amount", header: "金额", width: "120px", align: "right", render: (r) => <span className="text-sm font-medium text-[#2E8BFF]">{formatCurrency(r.total_amount, r.currency)}</span> },
  ...
  const quotationFormConfig = {
    ...
    itemLabel: "报价明细",
    onSubmit: async (data: any) => { await createQuotation(data); },
  };

  return (
    <div>
      <GenericFormDialog open={createOpen} onOpenChange={setCreateOpen} config={quotationFormConfig} onSuccess={() => window.location.reload()} />
      <PageHeader title="报价管理" description={`共 ${filtered.length} 条报价记录`} actions={<>
        ...
        <ActionButton icon="add" onClick={() => setCreateOpen(true)}>新建报价</ActionButton>
      </>
      } />
