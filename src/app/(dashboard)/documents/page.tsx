...
export default function DocumentsPage() {
  ...
  const columns: Column<Document>[] = [
    { key: "code", header: "单证编号", width: "140px", render: (r) => <Link href={`/documents/${r.id}`} className="text-xs font-mono text-[#2E8BFF] hover:underline">{r.code}</Link> },
    { key: "order_code", header: "关联订单", width: "140px", render: (r) => r.order_code ? <Link href={`/orders/${r.order_id}`} className="text-xs font-mono text-[#2E8BFF] hover:underline">{r.order_code}</Link> : "-" },
    ...
    { key: "actions", header: "操作", width: "180px", align: "center", render: (r) => (
      <div className="flex items-center justify-center gap-1">
        ...
        {r.status === "draft" && <Button size="sm" className="h-7 bg-[#2E8BFF] hover:bg-[#0B4BB8] text-xs" onClick={(e) => { e.stopPropagation(); handleGenerate(r); }}>生成PDF</Button>}
