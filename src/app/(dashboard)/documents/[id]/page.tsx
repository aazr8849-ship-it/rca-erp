      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <InfoCard title="单证信息" icon={FolderOpen} className="lg:col-span-2">
          <div className="grid grid-cols-2 gap-x-4 gap-y-4">
            ...
            <InfoItem label="关联订单" value={doc.order_code ? <Link href={`/orders/${doc.order_id}`} className="text-[#2E8BFF] hover:underline">{doc.order_code}</Link> : "-"} />
        ...
        {order && (
          <InfoCard title="关联订单">
            <div className="space-y-2 text-sm">
              <div><div className="text-xs text-gray-500">订单号</div><Link href={`/orders/${order.id}`} className="font-medium text-[#2E8BFF] hover:underline">{order.code}</Link></div>
