      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {statusFlow[inquiry.status]?.map((to) => (
            <Button key={to} onClick={() => handleStatusChange(to)} variant={to === "cancelled" ? "destructive" : "default"} className={to !== "cancelled" ? "bg-[#2E8BFF] hover:bg-[#0B4BB8]" : ""}>
              {statusLabels[to]}
            </Button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <InfoCard title="询盘信息" icon={MessageSquare} className="lg:col-span-2">
          <div className="grid grid-cols-2 gap-x-4 gap-y-4">
            ...
            <InfoItem label="客户" value={<Link href={`/customers/${inquiry.customer_id}`} className="text-[#2E8BFF] hover:underline">{inquiry.customer_name}</Link>} />
          ...
          <div className="space-y-2">
            {relatedQuotations.length === 0 ? <div className="text-center py-6 text-sm text-gray-500">暂无关联报价</div> : relatedQuotations.map((q) => (
              <Link key={q.id} href={`/quotations/${q.id}`} className="flex items-center justify-between p-2.5 rounded border border-gray-100 hover:bg-gray-50">
                <div><div className="text-xs font-mono text-[#2E8BFF]">{q.code}</div><div className="text-xs text-gray-500">{formatCurrency(q.total_amount, q.currency)}</div></div>
