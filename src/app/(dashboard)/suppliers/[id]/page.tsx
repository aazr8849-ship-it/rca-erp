  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Button onClick={() => router.push("/suppliers")} className="bg-[#3298cb] hover:bg-[#2c87b3]"><Pencil className="h-3.5 w-3.5 mr-1.5" />编辑信息</Button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <InfoCard title="基本信息" icon={Building2} className="lg:col-span-2">
          <div className="grid grid-cols-2 gap-x-4 gap-y-4">
            ...
            <InfoItem label="联系邮箱" value={supplier.contact_email ? <a href={`mailto:${supplier.contact_email}`} className="text-[#3298cb] hover:underline flex items-center gap-1"><Mail className="h-3 w-3" />{supplier.contact_email}</a> : "-"} />
        ...
        <InfoCard title="业务概览">
          <div className="space-y-3">
            <div className="bg-blue-50 rounded-md p-3"><div className="text-xs text-gray-600">采购订单</div><div className="text-2xl font-bold text-[#3298cb] mt-1">{supplierPOs.length}</div></div>
      ...
      <Tabs defaultValue="products">
        ...
        <TabsContent value="products" className="mt-3">
          <GenericDataTable data={supplierProducts} columns={[
            { key: "code", header: "编码", render: (r) => <Link href={`/products/${r.id}`} className="text-[#3298cb] hover:underline text-xs font-mono">{r.code}</Link> },
        ...
        <TabsContent value="purchases" className="mt-3">
          <GenericDataTable data={supplierPOs} columns={[
            { key: "code", header: "采购单号", render: (r) => <Link href={`/purchases/${r.id}`} className="text-[#3298cb] hover:underline text-xs font-mono">{r.code}</Link> },
