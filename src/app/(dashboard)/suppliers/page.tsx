...
export default function SuppliersPage() {
  ...
  const columns: Column<Supplier>[] = [
    ...
    { key: "name", header: "供应商名称", render: (r) => <Link href={`/suppliers/${r.id}`} className="text-sm font-medium text-[#2E8BFF] hover:underline">{r.name}</Link> },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter>
          ...
          <Button onClick={() => { if (!form.name || !form.contact_person || !form.country) { toast.error("请填写必填项"); return; } onSave(form); }} className="bg-[#2E8BFF] hover:bg-[#0B4BB8]">{supplier ? "保存修改" : "创建供应商"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
