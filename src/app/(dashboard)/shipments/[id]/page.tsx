      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <InfoCard title="发货信息" icon={Ship} className="lg:col-span-2">
          <div className="grid grid-cols-2 gap-x-4 gap-y-4">
            ...
            <InfoItem label="关联订单" value={shipment.order_code ? <Link href={`/orders/${shipment.order_id}`} className="text-[#2E8BFF] hover:underline">{shipment.order_code}</Link> : "-"} />
        ...
        {customer && (
          <InfoCard title="收货客户">
            <div className="space-y-2 text-sm">
              <div><div className="text-xs text-gray-500">客户名称</div><Link href={`/customers/${customer.id}`} className="font-medium text-[#2E8BFF] hover:underline">{customer.name}</Link></div>
